...
import { decentpropmate_backend } from "../../declarations/decentpropmate_backend";

//1. LOCAL DATA
const pollResults = {
    "Modernizar el elevador": 0,
    "Ampliar el gimnasio": 0,
    "Modernizar el teatro": 0,
    "Modernizar el sistema de seguridad": 0
};
...
document.addEventListener('DOMContentLoaded', async (e) => {
  ...
  const question = await decentpropmate_backend.getQuestion();
  ...
  const voteCounts = await decentpropmate_backend.getVotes();
...
pollForm.addEventListener('submit', async (e) => {
  ...
  const updatedVoteCounts = await decentpropmate_backend.vote(checkedValue);
...
resetButton.addEventListener('click', async (e) => {
    ...
    await decentpropmate_backend.resetVotes();
    const voteCounts = await decentpropmate_backend.getVotes();
    updateLocalVoteCounts(voteCounts);
...
// Mocking the backend methods
/*const serviceRequests = [];

function calculateTotalCost(requests) {
  let totalCost = 0;
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  serviceRequestForm.addEventListener('submit', (e) => {
    ...
    const tenantName = e.target.elements.tenantName.value;
    const aptNumber = parseInt(e.target.elements.aptNumber.value);
    ...
    const totalCost = calculateTotalCost([newRequest]);
